...
def walletdataframe(wallet, block):
    ...
    # NOTE: Do some maths
    df['balance'] = df['token_address'].apply(getbalance) / 10 ** df['decimals'].apply(
        int
    )
    ...
    ethbal = (
        web3.eth.get_balance(convert.to_address(wallet), block_identifier=block)
        / 10 ** 18
    )
    if ethbal > 0:
...
def dataframe(block):
    ...
    debt = unitVault.getTotalDebt(
        '0x0bc529c00C6401aEF6D220BE8C6Ea1667F6Ad93e', ychad, block_identifier=block
    )
    if debt > 0:
        df = appendmore(
    ...
    bal = escrow.userLiquidityTotalAmount(
        yearnKp3rWallet, kp3rLPtoken, block_identifier=block
    )
    if bal > 0:
        df = appendmore(
            kp3rLPtoken.address,
