...
from yearn.apy.common import (SECONDS_PER_YEAR, Apy, ApyError, ApyFees,
                              ApySamples, SharePricePoint, calculate_roi, get_samples)
    ...
    if hasattr(strategy_contract, "pid"):
        ...
        if rewards_length > 0:
            for x in range(rewards_length):
                ...
                if virtual_rewards_pool.periodFinish() > current_time and virtual_rewards_pool.rewardToken() == cvx.address:
                    reward_apr += (virtual_rewards_pool.rewardRate() * SECONDS_PER_YEAR * cvx_price) / (base_asset_price * pool_price * virtual_rewards_pool.totalSupply())
