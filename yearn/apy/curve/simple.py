...
def simple(strategy) -> StrategyApy:
    ...
    if hasattr(gauge, "reward_contract"):
        ...
        for i in range(gauge.reward_count()):
            ...
            if gauge_reward_token == addresses[chain.id]['rkp3r_rewards']:
                ...
                token_price = (
                    get_price(addresses[chain.id]['kp3r'], block=block)
                    * (100 - discount)
                    / 100
                )
            else:
    ...
    strategy_performance_fee = (
        vault_contract.strategies(strategy_contract)['performanceFee'] / 1e4
    )
    vault_performance_fee = (
        (vault_contract.performanceFee(block_identifier=block)) / 1e4
    ...
    if hasattr(strategy_contract, "pid"):
        ...
        if rewards_length > 0:
            for x in range(rewards_length):
                print("This is our first x value:", x)
                virtual_rewards_pool = contract(rewards_contract.extraRewards(x))
                # we only want active CVX rewards for now, this is all they are adding extra other than those that come through curve already
                if (
                    virtual_rewards_pool.periodFinish() < current_time
                    or virtual_rewards_pool.rewardToken() != cvx.address
                ):
                    continue
                reward_apr += (
                    virtual_rewards_pool.rewardRate() * SECONDS_PER_YEAR * cvx_price
                ) / (base_asset_price * pool_price * virtual_rewards_pool.totalSupply())

        gross_apr = ((1 - cvx_fee) * cvx_boost * base_apr) * (
            1 + cvx_printed_as_crv
        ) + reward_apr
        cvx_boosted_apr = (1 - cvx_fee) * cvx_boost * base_apr
        cvx_apr_minus_keep_crv = ((1 - cvx_fee) * cvx_boost * base_apr) * (
            (1 - keep_crv) + cvx_printed_as_crv
        )
        ...
        # calculate our net apy
        cvx_net_apr = (cvx_apr_minus_keep_crv + reward_apr) * (
            1 - performance
        ) - management
        ...
        fees = ApyFees(
            performance=performance, management=management, keep_crv=keep_crv
        )
        return StrategyApy("convex", gross_apr, net_apy, fees, composite=composite)
    ...
    return StrategyApy("curve", gross_apr, net_apy, fees, composite=composite)
