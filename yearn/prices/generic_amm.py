...
class GenericAmm:
    ...
    def is_generic_amm(self, lp_token_address: str) -> bool:
        try:
            ...
            return all(
                hasattr(token_contract, attr)
                for attr in ['getReserves', 'token0', 'token1']
            )
    ...
    def get_price(self, lp_token_address: str, block: int = None) -> float:
        ...
        total_supply, decimals = fetch_multicall(
            *[[lp_token_contract, attr] for attr in ['totalSupply', 'decimals']],
            block=block
        )
    ...
    def get_tokens(self, lp_token_address: str, block: int = None):
        ...
        return fetch_multicall(
            *[[lp_token_contract, attr] for attr in ['token0', 'token1']]
        )
    ...
    def get_tvl(self, lp_token_address: str, block: int = None) -> float:
        ...
        reserves = [
            reserves[i] / 10 ** contract(token).decimals()
            for i, token in enumerate(tokens)
        ]
        return sum(
            reserve * magic.get_price(token) for token, reserve in zip(tokens, reserves)
        )
