...
from yearn.outputs.postgres.tables import (
    addresses_table_query,
    tokens_table_query,
...
POSTRGES_CONN_STRING = (
    "Driver={PostgreSQL Unicode};Server=postgres;Port:5432;"
...
class PostgresInstance:
    ...
    def table_exists(self, table_name):
        response = self.cursor.execute(
            f"select exists(select tablename from pg_tables where tablename = '{table_name}')"
        ).fetchone()[0]
    ...
    def last_recorded_block(self, table_name):
        ...
        '''
        response = self.cursor.execute(
            f'SELECT max(block) from {table_name} where chainid = {chainid}'
        ).fetchone()[0]
        self.conn.commit()
        return response
    ...
    def token_exists(self, token_address: str):
        response = self.cursor.execute(
            f"select exists(select symbol from tokens where token_address = '{token_address}' and chainid = {chainid})"
        ).fetchone()[0]
        return True if response == '1' else False
    ...
    def address_exists(self, address: str):
        response = self.cursor.execute(
            f"select exists(select address from addresses where address = '{address}' and chainid = {chainid})"
        ).fetchone()[0]
        return True if response == '1' else False
        ...
        while i < 10:
            try:
                if not self.address_exists(address):
                    self.cursor.execute(
                        f"INSERT INTO ADDRESSES (CHAINID, ADDRESS, IS_CONTRACT)\
    ...
    def fetch_balances(self, vault_address, block):
        ...
        self.conn.commit()
        return {
            wallet: balance for wallet, balance in balances if wallet != ZERO_ADDRESS
        }
