        ...
        for vault, params in data[product].items():
            ...
            for key, value in params.items():
                ...
                if (
                    product == "ib"
                    and key == 'tvl'
                    and block >= constants.ib_snapshot_block
                ):
                    # create one item with tvl=0 that will be used in existing dashboards
                    item_legacy = _build_item(
                        metric, label_names, label_values, 0, timestamp
                    )
                    metrics_to_export.append(item_legacy)
                    # create a second item to track ib tvl separately
                    item_own = _build_item(
                        f'{metric}_own', label_names, label_values, value, timestamp
                    )
                    metrics_to_export.append(item_own)
                else:
                    item = _build_item(
                        metric, label_names, label_values, value, timestamp
                    )
