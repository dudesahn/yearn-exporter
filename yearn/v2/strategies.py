...
from yearn.apy.common import StrategyApy, ApyFees, ApyError, ApySamples
...
class Strategy:
    ...
    def harvests_data(self):
        ...
        sorted_harvests = sorted(
            self._harvests_data, key=lambda k: k['block_number'], reverse=True
        )
    ...
    def debt_ratio(self):
        return (
            self.vault.vault.strategies(self.strategy.address).dict()['debtRatio'] / 1e4
        )
    ...
    def apy(self) -> StrategyApy:
        ...
        strategy_performance_fee = (
            vault.strategies(strategy.address).dict()['performanceFee'] / 1e4
        )
        vault_performance_fee = vault.performanceFee() / 1e4
        performance_fee = vault_performance_fee + strategy_performance_fee
        management_fee = (
            vault.managementFee() / 1e4 if hasattr(vault, "managementFee") else 0
        )
        ...
        if profitable_harvests_count < 2:
            return StrategyApy("too_few_profitable", 0, 0, fees)

        # Not enough data
        have_enough_data = second_latest_harvest_with_profit_idx + 1 < len(harvests)
        if have_enough_data == False:
            return StrategyApy("too_little_data", 0, 0, fees)

        # Latest profitable harvest
        latest_harvest_with_profit_current = harvests[latest_harvest_with_profit_idx]
        second_latest_harvest_with_profit_current = harvests[
            latest_harvest_with_profit_idx + 1
        ]
        harvest_profit_current = latest_harvest_with_profit_current['event']['profit']
        seconds_between_harvests_current = (
            chain[latest_harvest_with_profit_current['block_number']]['timestamp']
            - chain[second_latest_harvest_with_profit_current['block_number']][
                'timestamp'
            ]
        )
        block_before_profitable_harvest_current = (
            latest_harvest_with_profit_current['block_number'] - 1
        )
        strategy_debt_current = vault.strategies(
            strategy.address, block_identifier=block_before_profitable_harvest_current
        ).dict()['totalDebt']
        if strategy_debt_current == 0:
            current_apr = 0
        else:
            current_apr = (
                harvest_profit_current
                / strategy_debt_current
                * (SECONDS_IN_YEAR / seconds_between_harvests_current)
            )

        # Second latest profitiable harvest
        latest_harvest_with_profit_previous = harvests[
            second_latest_harvest_with_profit_idx
        ]
        second_latest_harvest_with_profit_previous = harvests[
            second_latest_harvest_with_profit_idx + 1
        ]
        harvest_profit_previous = latest_harvest_with_profit_previous['event']['profit']
        seconds_between_harvests_previous = (
            chain[latest_harvest_with_profit_previous['block_number']]['timestamp']
            - chain[second_latest_harvest_with_profit_previous['block_number']][
                'timestamp'
            ]
        )
        block_before_profitable_harvest_previous = (
            latest_harvest_with_profit_previous['block_number'] - 1
        )
        strategy_debt_previous = vault.strategies(
            strategy.address, block_identifier=block_before_profitable_harvest_previous
        ).dict()['totalDebt']
        if strategy_debt_previous == 0:
            previous_apr = 0
        else:
            previous_apr = (
                harvest_profit_previous
                / strategy_debt_previous
                * (SECONDS_IN_YEAR / seconds_between_harvests_previous)
            )
        ...
        return StrategyApy("harvest", apr, net_apy, fees)
